...
![2025_02_25_08_50_26.png](./assets/AndroidImg/1740444720278-6a0fb706-d434-4712-933c-88c6e00f3da5.webp)
...
![image2113](./assets/AndroidImg/image2113.png)
...
![img](./assets/AndroidImg/dg_viewgroup_hierarchy.png)
...
![img](./assets/AndroidImg/dg_common_layouts_visual_rep.png)
...
![img](./assets/AndroidImg/dg_layout_diagram_and_hierarchy.png)
...
![img](./assets/AndroidImg/as_activity_main_in_project_pane_annot.png)
...
- `Padding` 是 UI 元素边缘与元素内容之间的空间，以与密度无关的像素为单位，如下图所示。
  ![img](./assets/AndroidImg/dg_padding_annotated.png)
...
您还应该*将* XML 布局文件中的硬编码字符串提取到字符串资源中。![ Extracting a string resource](./assets/AndroidImg/as_extract_string_resources.png)

![ Naming the string resource](./assets/AndroidImg/as_extract_string_resources2.png)
...
您可以在 `colors.xml` 中颜色资源声明旁边的左旁注中，以及使用布局 XML 文件中资源名称的属性旁边的左旁注中看到一小块颜色选择。![ Color blocks in the resource file](./assets/AndroidImg/as_color_block_in_colors_xml.png)

![ Color block in the layout file](./assets/AndroidImg/as_color_block_in_layout.png)
...
要在屏幕上显示可滚动的杂志文章，可以使用 [`RelativeLayout`](https://developer.android.com/reference/android/widget/RelativeLayout.html)，其中包括一个单独的 TextView 用于文章标题，另一个 `TextView` 用于文章副标题，第三个 `TextView` 用于滚动文章文本（见下图），在 `ScrollView` 中设置。屏幕中唯一会滚动的部分是带有文章文本的 `ScrollView`。![ The layout with a ScrollView](./assets/AndroidImg/dg_layout_diagram1.png)
...
例如，如果您希望文章的副标题随文章一起滚动，即使它们是单独的 `TextView` 元素，请将 `LinearLayout` 作为单个子 `View` 添加到 `ScrollView` 中，如下图所示，然后将 `TextView` 副标题和 article 元素移动到 `LinearLayout` 中。用户滚动整个 `LinearLayout`，其中包括副标题和文章。![ A LinearLayout Inside the ScrollView](./assets/AndroidImg/dg_layout_diagram2.png)
