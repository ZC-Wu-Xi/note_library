...
[code](./src/main/java/com/xi/day4/analysis/AnalysisTest.java)

数据源：[data.txt文件](./data.txt)
...
```java
try (Stream<String> lines = Files.lines(Path.of("./data.txt"))) { // Path.of(): 获取文件的流数据;  Files.lines: 接受一个 Path 参数，并返回一个 Stream<String>，该流按行读取文本文件中的数据。每一行文本都被表示为一个字符串。
    lines.skip(1).limit(5).forEach(System.out::println); // 跳过第一个，截取前 5 个元素，即前2-6行
} catch (IOException e) {
    throw new RuntimeException(e);
}
```
...
按日期顺序统计每月的销售量，即按日期分组统计个数

~~其中 1970-01 应该是数据的问题，我们就不做更改了~~

```java
try (Stream<String> lines = Files.lines(Path.of("./data.txt"))) { // Path.of(): 获取文件的流数据;  Files.lines: 接受一个 Path 参数，并返回一个 Stream<String>，该流按行读取文本文件中的数据。每一行文本都被表示为一个字符串。
    lines.skip(1) // 跳过第一行即跳过第一个元素
        .map((line) -> line.split(",")) // 把字符串根据"，"分割为数组
        //                    .collect(groupingBy(array -> YearMonth.from(formatter.parse(array[TIME])), counting()));// 收集器 根据年月分组 无序
        .collect(groupingBy(array -> YearMonth.from(formatter.parse(array[TIME])), TreeMap::new, counting())) // 收集器 根据年月分组 使用treeMap收集有序
        .forEach((k, v) -> System.out.println(k + " 订单数 " + v));
} catch (IOException e) {
    throw new RuntimeException(e);
}
```
...
> 补充扩展：
>
> `TreeMap` 是 Java Collections Framework 中的一个实现了 `Map` 接口的类，具有以下几个主要特点：
>
> 1. **有序性**：`TreeMap` 是一个有序的映射，所有的键值对都是按照键的自然顺序或者构造 `TreeMap` 时提供的比较器的顺序进行排序的。这使得 `TreeMap` 可以有效地支持按顺序访问元素。
> 2. **基于红黑树**：`TreeMap` 的底层实现是自平衡的红黑树，因此它的插入、删除和查找操作的时间复杂度为 O(log n)，相较于 `HashMap` 的 O(1) 性能，`TreeMap` 在最坏情况下提供了更为稳定的性能。
> 3. **键的唯一性**：和其他的 `Map` 实现一样，`TreeMap` 的键是唯一的。如果将一个已经存在的键插入到 `TreeMap` 中，新的值将会覆盖原来的值。
> 4. **支持范围视图**：`TreeMap` 提供了一些方法来获取范围视图，例如 `subMap()`, `headMap()`, 和 `tailMap()`，允许用户在特定的键范围内处理元素。
> 5. **线程不安全**：`TreeMap` 不是线程安全的，如果在多个线程同时访问 `TreeMap` 时，推荐使用 `Collections.synchronizedSortedMap` 方法将其包装为线程安全的映射。
> 6. **允许 null 值**：`TreeMap` 允许使用 `null` 值，但对于键不允许使用 `null`，因为 `null` 键无法进行比较。
> 7. **实现了 `NavigableMap` 接口**：`TreeMap` 实现了 `NavigableMap` 接口，提供了额外的方法来支持类似导航的操作，比如获取上一个、下一个键等功能。
>
> 这些特点使得 `TreeMap` 在需要保持键的顺序、支持排序及范围查询时，是一个非常有用的选择。
...
try (Stream<String> lines = Files.lines(Path.of("./data.txt"))) {
    // 根据年月分组统计订单数
    Map<YearMonth, Long> collect = lines.skip(1) // 跳过第一行即跳过第一个元素
        .map((line) -> line.split(",")) // 把字符串根据"，"分割为数组
        .collect(groupingBy(array -> YearMonth.from(formatter.parse(array[TIME])), TreeMap::new, counting()));// 收集器 根据年月分组 使用treeMap收集有
    // 获取每月最大订单数的月份及订单数
    Optional<Map.Entry<YearMonth, Long>> max = collect.entrySet().stream()
        .max(Comparator.comparingLong(e -> e.getValue()));// 根据 value 进行比较

    // 打印
    //            max.ifPresent(entry -> { // 如果存在最大值 打印
    //                YearMonth yearMonth = entry.getKey();
    //                Long value = entry.getValue();
    //                System.out.println("YearMonth: " + yearMonth + ", Value: " + value); // YearMonth: 2020-10, Value: 104063
    //            });
    
    //            System.out.println(max.get()); // 2020-10=104063
    max.ifPresentOrElse( // max 值如果存在...，否则 ...
        entry -> System.out.println("键: " + entry.getKey() + ", 值: " + entry.getValue()), // max值存在时执行
        () -> System.out.println(0) // 默认值  不存在时执行
    );
} catch (IOException e) {
    throw new RuntimeException(e);
}
...
//        .max(Comparator.comparingLong(Map.Entry::getValue))
        ...
        .orElse(null);
    System.out.println(yearMonthLongEntry); // 2020-10=104063
} catch (IOException e) {
    throw new RuntimeException(e);
}
```
...
try(Stream<String> lines = Files.lines(Paths.get("./data.txt"))) {
    lines.skip(1)
        .map((line) -> line.split(","))// 跳过第一个元素
        .collect(groupingBy(array -> array[PRODUCT_ID], counting())) // 商品 id 为键，商品数量为值，收集为map
        .entrySet().stream() // 将map变为流
        .max(Map.Entry.comparingByValue()) // 根据商品数量进行比较获取最大值
        .ifPresentOrElse(System.out::println, () -> System.out.println("没有值或没有最大值"));
} catch (IOException e) {
    throw new RuntimeException(e);
}
```
...
try (Stream<String> lines = Files.lines(Path.of("./data.txt"))){
    lines.skip(1)
        ...
        .entrySet().stream()
        // .sorted(Comparator.comparingInt(u -> -Math.toIntExact(u.getValue()))) // 根据值进行比较正序，加上负号变为倒序
        // .sorted(Comparator.comparingLong((Map.Entry<String, Long> u) -> u.getValue()).reversed()) // reversed()将正序变为倒序
        .sorted(Map.Entry.<String, Long>comparingByValue().reversed()) // 根据值进行比较 <String, Long>告诉他泛型类型，不然无法推断出来
        .limit(10) // 取前十个
        .forEach(System.out::println);
} catch (IOException e) {
    throw new RuntimeException(e);
...
```java
1.515915625512423e+18=1092
1.5159156255121183e+18=1073
1.515915625512378e+18=1040
1.515915625512377e+18=1028
1.5159156255136955e+18=1002
1.515915625512422e+18=957
1.515915625513446e+18=957
1.515915625513447e+18=928
1.515915625514598e+18=885
1.5159156255147195e+18=869
```
